export class AgentManager {
    ...
    startAgentTask(id) {
        ...
        if (this.onTaskStart) {
            this.onTaskStart(agent.id, agent.prompt, agent.section)
                .then((result) => {
                    agent.status = "done";
    ...
    getStatusHtml(status) {
        switch (status) {
            case "working": return `<span class="badge bg-warning text-dark"><span class="spinner-grow spinner-grow-sm me-1" style="width:6px;height:6px"></span>Working</span>`;
            case "done": return `<span class="badge bg-success">Done</span>`;
            case "error": return `<span class="badge bg-danger">Error</span>`;
            default: return `<span class="badge bg-secondary">Idle</span>`;
        }
    }
}
